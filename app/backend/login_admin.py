...
from app.backend.controllers.users import create_access_string, hash_access_string, create_access_token
...
def login_as_admin(response: Response):
    ...
    try:
        ...
        if not user:
            ...
            password_hashed: str = hashpw(admin_pass.encode("utf-8"), salt).decode("utf-8")
            ...
            add_new_user(email=admin_email, password_hash=password_hashed, access_string_hash=access_string_hashed)
        ...
        raise HTTPException(status_code=500, detail="An internal error occurred during admin login.")
