...
from app.backend.models.users import UserController, User, UserAlreadyExistsError
...
class AuthService:
    def __init__(self, user_controller: UserController):
        if not isinstance(user_controller, UserController):
            raise TypeError(
                "user_controller must be an instance of UserController class"
            )
    ...
    def register_user(self, email: str, password: str) -> tuple[User, str]:
        ...
        try:
            ...
            new_user = self.users.add_new_user(
                email=email,
    ...
    def get_user_from_token(self, token: str):
        try:
            payload = jwt.decode(
                token, settings.secret_pepper, algorithms=[settings.jwt_algorithm]
            )
    ...
    def request_password_reset(
        self, email: str, expires_delta=settings.password_reset_token_lifetime
    ):
        user = self.users.find_user_by_email(email=email)
        if not user:
    ...
    def _send_password_reset_email(
        ...
        with open(
            "../../email_templates/password_reset.html", "r", encoding="utf-8"
        ) as email_template:
        ...
        text = html_template.format(
            application_server_url=config.application_server_url,
...
if __name__ == "__main__":
    AuthService._send_password_reset_email(
        "mnogoboksov21@gmail.com", "jdlksafj", timedelta(hours=1)
    )
