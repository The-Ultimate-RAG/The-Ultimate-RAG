...
from app.backend.exceptions import DatabaseError, UserNotFoundError, UserAlreadyExistsError
...
class UserController:
    ...
    def add_new_user(self, email: str, password_hash: str, access_string_hash: str) -> User:
        ...
        try:
            ...
            logging.error(f"Integrity error when adding user '{email}': {e}", exc_info=True)
    ...
    def find_user_by_access_string(self, access_string_hash: str) -> User | None:
        query = self.database.query(User).filter(User.access_string_hash == access_string_hash)
        return self._execute_query(query)
    ...
    def update_user(self, user_id: int, **kwargs) -> User:
        ...
        for key, value in kwargs.items():
            if key in allowed_updates:
                ...
                logging.warning(f"Attempted to updated disallowed key: {key} for user {user_id}. Ignoring")
        ...
        try:
            ...
            logging.error(f"Failed to update user ID {user_id} due to a database error: {e}", exc_info=True)
